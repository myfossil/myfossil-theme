			...
			<fieldset class="bbp-form">
				...
				<?php if ( !bbp_is_topic_open() && !bbp_is_reply_edit() ) : ?>
					...
					<div class="bbp-template-notice alert alert-info">
						<p><?php _e( 'This topic is marked as closed to new replies, however your posting capabilities still allow you to do so.', 'myfossil' ); ?></p>
					</div>
					...
					<?php if ( bbp_allow_revisions() && bbp_is_reply_edit() ) : ?>
						...
						<fieldset class="bbp-form">
							...
							<legend>
								...
								<label for="bbp_log_reply_edit"><?php _e( 'Keep a log of this edit:', 'myfossil' ); ?></label><br />
							</legend>
...
<?php elseif ( bbp_is_topic_closed() ) : ?>
	...
	<div id="no-reply-<?php bbp_topic_id(); ?>" class="bbp-no-reply">
		<div class="bbp-template-notice alert alert-info">
			<p><?php printf( __( 'The topic &#8216;%s&#8217; is closed to new replies.', 'myfossil' ), bbp_get_topic_title() ); ?></p>
		</div>
	</div>
...
<?php elseif ( bbp_is_forum_closed( bbp_get_topic_forum_id() ) ) : ?>
	...
	<div id="no-reply-<?php bbp_topic_id(); ?>" class="bbp-no-reply">
		<div class="bbp-template-notice alert alert-info">
			<p><?php printf( __( 'The forum &#8216;%s&#8217; is closed to new topics and replies.', 'myfossil' ), bbp_get_forum_title( bbp_get_topic_forum_id() ) ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-reply-<?php bbp_topic_id(); ?>" class="bbp-no-reply">
		<div class="bbp-template-notice alert alert-info">
			<p><?php is_user_logged_in() ? _e( 'You cannot reply to this topic.', 'myfossil' ) : _e( 'You must be logged in to reply to this topic.', 'myfossil' ); ?></p>
		</div>
	</div>
