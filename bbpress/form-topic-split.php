		...
		<div id="split-topic-<?php bbp_topic_id(); ?>" class="bbp-topic-split well">
			...
			<form id="split_topic" name="split_topic" method="post" action="<?php the_permalink(); ?>">
				...
				<fieldset class="bbp-form">
					...
					<legend><?php printf( __( 'Split topic "%s"', 'myfossil' ), bbp_get_topic_title() ); ?></legend>

					<div>

						<div class="bbp-template-notice alert alert-info info">
							<p><?php _e( 'When you split a topic, you are slicing it in half starting with the reply you just selected. Choose to use that reply as a new topic with a new title, or merge those replies into an existing topic.', 'myfossil' ); ?></p>
						</div>

						<div class="bbp-template-notice alert alert-info">
							<p><?php _e( 'If you use the existing topic option, replies within both topics will be merged chronologically. The order of the merged replies is based on the time and date they were posted.', 'myfossil' ); ?></p>
						</div>

						<fieldset class="bbp-form">
							<legend><?php _e( 'Split Method', 'myfossil' ); ?></legend>
							...
							<div>
								...
								<label for="bbp_topic_split_option_reply"><?php printf( __( 'New topic in <strong>%s</strong> titled:', 'myfossil' ), bbp_get_forum_title( bbp_get_topic_forum_id( bbp_get_topic_id() ) ) ); ?></label>
								<input type="text" id="bbp_topic_split_destination_title" value="<?php printf( __( 'Split: %s', 'myfossil' ), bbp_get_topic_title() ); ?>" tabindex="<?php bbp_tab_index(); ?>" size="35" name="bbp_topic_split_destination_title" />
							</div>
							...
							<?php if ( bbp_has_topics( array( 'show_stickies' => false, 'post_parent' => bbp_get_topic_forum_id( bbp_get_topic_id() ), 'post__not_in' => array( bbp_get_topic_id() ) ) ) ) : ?>
								...
								<div>
									...
									<label for="bbp_topic_split_option_existing"><?php _e( 'Use an existing topic in this forum:', 'myfossil' ); ?></label>
						...
						<fieldset class="bbp-form">
							...
							<div>
								...
								<?php if ( bbp_allow_topic_tags() ) : ?>
									...
									<label for="bbp_topic_tags"><?php _e( 'Copy topic tags to the new topic', 'myfossil' ); ?></label><br />
