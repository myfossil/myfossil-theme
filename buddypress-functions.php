...
function bp_legacy_theme_ajax_joinleave_group()
{
    ...
    if (!$group = groups_get_group(array(
        'group_id' => $group_id
    ))) return;
...
function bp_legacy_theme_ajax_messages_send_reply()
{
    ...
    if (!empty($result)) {
        ...
        bp_loggedin_user_fullname(); ?></a> <span class="activity"><?php
        printf(__('Sent %s', 'buddypress') , bp_core_time_since(bp_core_current_time())); ?></span></strong>
