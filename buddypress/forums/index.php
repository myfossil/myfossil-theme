...
if (is_user_logged_in() && bp_get_forum_topic_count_for_user(bp_loggedin_user_id())): ?>
    ...
    echo trailingslashit(bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics'); ?>"><?php
    printf(__('My Topics <span>%s</span>', 'buddypress') , bp_get_forum_topic_count_for_user(bp_loggedin_user_id())); ?></a></li>
...
if (is_user_logged_in()): ?>
    ...
    if (bp_is_active('groups') && bp_has_groups('user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100')): ?>
        ...
        do_action('groups_forum_new_topic_before'); ?>
					...
					<a name="post-new"></a>
					<h5><?php
        _e('Create New Topic:', 'buddypress'); ?></h5>
        ...
        _e('Tags (comma separated):', 'buddypress'); ?></label>
					...
					<label><?php
        _e('Post In Group Forum:', 'buddypress'); ?></label>
					<select id="topic_group_id" name="topic_group_id">

						<option value=""><?php /* translators: no option picked in select box */

        _e('----', 'buddypress'); ?></option>
